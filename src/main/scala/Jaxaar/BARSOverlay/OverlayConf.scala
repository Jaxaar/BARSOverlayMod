...
class OverlayColumnValues(val title: String, val fieldLength: Int, val getString: HypixelPlayerData => String, val specialFormat: Boolean = false, val colorBreakpoints: List[Double] = List(), val getValue: HypixelPlayerData => Double = (p) => 0){
